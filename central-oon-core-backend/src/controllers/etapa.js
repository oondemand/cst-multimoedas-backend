...
const criarEtapa = async (req, res) => {
  ...
  sendResponse({ res, statusCode: 201, etapa });
};

const listarEtapas = async (req, res) => {
  const { pageIndex, pageSize, searchTerm, ...rest } = req.query;

  const { etapas, limite, totalDeEtapas, page } = await EtapaService.listarComPaginacao({
    filtros: rest,
    pageIndex,
    pageSize,
    searchTerm,
...
const listarEtapasAtivasPorEsteira = async (req, res) => {
  ...
  sendResponse({ res, statusCode: 200, etapas });
};

const atualizarEtapa = async (req, res) => {
  const etapa = await EtapaService.atualizar({ etapa: req.body, id: req.params.id });
  sendResponse({ res, statusCode: 200, etapa });
};
